...
import { BaseFileSystem, SynchronousFileSystem } from "../file_system";
...
export class SyncKeyValueFileSystem extends SynchronousFileSystem {
    ...
    renameSync(oldPath, newPath) {
        const tx = this.store.beginTransaction("readwrite"); const oldParent = path.dirname(oldPath); const oldName = path.basename(oldPath); const newParent = path.dirname(newPath); const newName = path.basename(newPath);
        // Remove oldPath from parent's directory listing.
        const oldDirNode = this.findINode(tx, oldParent); const oldDirList = this.getDirListing(tx, oldParent, oldDirNode);
    ...
    createFileSync(p, flag, mode) {
        const tx = this.store.beginTransaction("readwrite"); const data = emptyBuffer(); const newFile = this.commitNewFile(tx, p, FileType.FILE, mode, data);
        // Open the file.
        return new SyncKeyValueFile(this, p, flag, newFile.toStats(), data);
    }

    openFileSync(p, flag) {
        const tx = this.store.beginTransaction("readonly"); const node = this.findINode(tx, p); const data = tx.get(node.id);
        if (data === undefined) {
    ...
    _syncSync(p, data, stats) {
        ...
        const fileInodeId = this._findINode(tx, path.dirname(p), path.basename(p)); const fileInode = this.getINode(tx, p, fileInodeId); const inodeChanged = fileInode.update(stats);
        try {
            // Sync data.
    ...
    makeRootDirectory() {
        ...
        if (tx.get(ROOT_NODE_ID) === undefined) {
            ...
            // Mode 0666
            const dirInode = new Inode(GenerateRandomID(), 4096, 511 | FileType.DIRECTORY, currTime, currTime, currTime);
    ...
    commitNewFile(tx, p, type, mode, data) {
        const parentDir = path.dirname(p); const fname = path.basename(p); const parentNode = this.findINode(tx, parentDir); const dirListing = this.getDirListing(tx, parentDir, parentNode); const currTime = (new Date()).getTime();
     ...
     */
    removeEntry(p, isDir) {
        const tx = this.store.beginTransaction("readwrite"); const parent = path.dirname(p); const parentNode = this.findINode(tx, parent); const parentListing = this.getDirListing(tx, parent, parentNode); const fileName = path.basename(p);
...
export class AsyncKeyValueFileSystem extends BaseFileSystem {
    ...
    rename(oldPath, newPath, cb) {
        ...
        const tx = this.store.beginTransaction("readwrite");
        const oldParent = path.dirname(oldPath); const oldName = path.basename(oldPath);
        const newParent = path.dirname(newPath); const newName = path.basename(newPath);
        const inodes = {};
        const lists = {};
        ...
        const theOleSwitcharoo = () => {
            ...
            const oldParentList = lists[oldParent]; const oldParentINode = inodes[oldParent]; const newParentList = lists[newParent]; const newParentINode = inodes[newParent];
    ...
    createFile(p, flag, mode, cb) {
        const tx = this.store.beginTransaction("readwrite"); const data = emptyBuffer();
    ...
    commitNewFile(tx, p, type, mode, data, cb) {
        const parentDir = path.dirname(p); const fname = path.basename(p); const currTime = (new Date()).getTime();
        // Invariant: The root always exists.
        // If we don't check this prior to taking steps below, we will create a
        // file with name '' in root should p == '/'.
        if (p === "/") {
    ...
    removeEntry(p, isDir, cb) {
        ...
        const tx = this.store.beginTransaction("readwrite"); const parent = path.dirname(p); const fileName = path.basename(p);
