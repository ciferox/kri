...
class App extends EventEmitter {
     ...
     */
    async setIcon(icon) {
        const buffer = is.string(icon) ? await fsReadFile(icon) : icon;
...
class Window extends EventEmitter {
    ...
    async init_() {
        ...
        await Promise.all([
            ...
            this.session_.send("Emulation.setDefaultBackgroundColorOverride",
                {
                    color: {
                        r: bgcolorRGBA[0], g: bgcolorRGBA[1],
    ...
    async configureRpcOnce_() {
        ...
        const rpcFile = (await fsReadFile(path.join(this.options_.rpcContentScriptDir, "rpc.js"))).toString();
        const features = [
...
async function launch(options = {}) {
    ...
    options.rpcContentScriptDir = options.rpcContentScriptDir || path.join(__dirname, "./rpc");
    ...
    if (is.number(options.width) && is.number(options.height)) {
        args.push(`--window-size=${options.width},${options.height}`);
    }
    if (is.number(options.left) && is.number(options.top)) {
